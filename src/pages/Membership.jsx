    ...
    <div className='w-full h-full font-aeonik overflow-x-hidden'>
        ...
        <div className='w-full'>
            <Hero image={TI} 
              ...
              url={"<a href='/' className='hover:underline'>Home</a> / Membership"}
            />
            <main className="about-main-header pt-10 w-full">
                <div className="main-inner hide-bar pl-[14rem] pr-3 md:px-0 overflow-x-auto min-full text-purple-light font-normal text-sm md:text-2xl gap-x-7 flex md:gap-x-12 justify-center border-b border-b-[#DBD8D1] cursor-pointer">
                  {tabs.map((e, index)=>(
                    <a href={e.tag} key={index}>
                      <h4 
                        className={`w-fit text-nowrap hover:opacity-60 capitalize  transition-all ease-linear duration-200 border-b-3 md:py-3 py-2 font-semibold text-dark ${index === btn? "border-b-purple": 'border-b-transparent'} `}
