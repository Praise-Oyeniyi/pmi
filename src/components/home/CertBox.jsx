...
const CertBox = ({image, title, mainTitle, subTitle, boxInfo, gradient, index, id}) => {
  return (
    <div className={`w-full group h-full overflow-hidden px-5 pb-5 md:pb-6 box-outer rounded-[1.25rem] bg-cover ${index === 0 && ' borde border-[#291843b4] '}`} style={{background:`url(${gradient})`}}>
        <div className="box-inner relative flex flex-col justify-between h-full">
            <div className=''>
                <div className='flex justify-between'>
                    <button className={`tag my-5 md:my-6 text-white border border-white rounded-full px-3 h-6 md:h-[1.875rem] text-sm md:text-base flex justify-center items-center w-fit ${index === 0 && ' border !border-[#291843] !text-[#291843] '}`}>Certification</button>
                ...
                <div className="box-body mt-2">
                    <h5 className={`text-base md:text-3xl font-bold py-2 ${index === 2? 'text-[#B365FD]': index ===1 ? 'text-[#05BFE0]':'text-[#200F3B]'}`}>{title}</h5>
