...
const CertBox = ({image, title, mainTitle, subTitle, boxInfo, gradient, titleColor}) => {
  return (
    <div className={`w-full h-fit md:h-[38rem] overflow-hidden px-4 md:px-5 pb-4 md:pb-6 box-outer rounded-[1.25rem] bg-gradient-to-b ${gradient} ${titleColor === 'text-[#200F3B]' && ' border border-[#291843b4] '}`}>
        <div className="box-inner relative flex flex-col justify-between h-full">
            <div>
                <div className='flex justify-between'>
                    <button className={`tag my-4 md:my-6 text-white border border-white rounded-full px-3 h-[1.875rem] text-sm md:text-base flex justify-center items-center w-fit ${titleColor === 'text-[#200F3B]' && ' border !border-[#291843] !text-[#291843] '}`}>Certification</button>
                ...
                <div className="box-body mt-2">
                    ...
                    <div className={`mt-2 md:mt-5`}>
                        ...
                        <p className={`font-normal text-sm md:text-lg text-off-white ${titleColor === 'text-[#200F3B]' && '!text-[#3A2B51] '}`}>{boxInfo}</p>
                    </div>
                </div>
            </div>
