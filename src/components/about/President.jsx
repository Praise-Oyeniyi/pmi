                ...
                <div>
                    {PP.map((e, index) =>(
                        <div className={`border-b py-5 border-b-[#DBD8D1] gap-y-4 md:gap-y-0 w-full flex md:flex-row flex-col items-center gap-x-10 justify-between ${index === PP.length-1 && 'border-none'}`} key={index}>
                            ...
                            <div className=' h-[9.6rem] w-full rounded-2xl overflow-hidden md:h-48 order-1 md:order-2'>
                                <img src={e.image} alt="pmi board of directors" className='h-full w-full object-cover object-top'/>
                            </div>
                        </div>
                    ))}
                </div>
