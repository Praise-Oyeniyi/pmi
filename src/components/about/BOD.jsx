  ...
  return (
    <div className='w-full'>
        <div className="mx-auto w-[90%] md:w-5/6 text-dark border-b border-b-[#DBD8D1] mb-10 pb-12" id='bod'>
            ...
            <p className='text-sm md:text-lg font-normal py-3 md:pb-7 text-center text-[#200F3B]'>
            ...
            <div className='w-full'>
                        ...
                        {bodData.map((e, index) =>(
                            <div className={`border-b py-5 border-b-[#DBD8D1] w-full flex flex-col gap-y-4 md:gap-y-0 md:flex-row  gap-x-10 justify-between items-center ${index === bodData?.length-1 && 'border-none'}`} key={e?.id}>
                                ...
                                <div className='flex flex-col md:flex-row  justify-between items-start w-full md:w-4/6 order-2 md:order-1'>
                                    <h4 className='text-lg md:text-2xl font-bold md:w-3/6 pr-10 tracking-tighter md:tracking-normal'>{e?.name}</h4>
                                    <div className="bodData-info text-sm md:text-lg pt-2 md:pt-0 font-normal space-y-2 md:w-3/6">
                                        ...
                                        <Link to={e?.linkedin} target='_blank'>
                                            <button className='flex gap-x-1 cursor-pointer md:gap-x-2 items-center font-bold text-sm md:text-base transition-all ease-in duration-300 hover:underline  underline-offset-3'>View Bio <FaChevronRight className='h-2.5 md:h-3.5'/></button>
                                        </Link>
                                    </div>
                                </div>
                ...
                <div className="other-bod md:flex justify-between items-center pt-3 md:pt-10 space-y-2 md:space-y-0">
                    <h4 className='text-base md:text-2xl font-normal'>Past Board Members</h4>
                    <div className='text-sm  md:text-lg space-y-2 capitalize'>
                        <p className='font-medium'>Explore the PMI Board of directors from present day to inception.</p>
