...
const Hero = ({image, title, duration, body, url, id, link}) => {
  ...
  return (
    <div>
        {data && certBG && (
            <div 
            className={`w-full hero-cert relative h-auto text-[#291843] overflow-hidden py-7 bg-cover bg-no-repeat bg-center ${title && title.toLowerCase().includes('pmp') && 'text-white'}`} 
                ...
                <div className="mx-auto w-[90%] md:w-5/6 ">
                    ...
                    <div className='w-full md:flex gap-x-10 space-y-7 md:space-y-0 mt-7'>
                        <div className='w-full md:w-3/6 md:h-[30rem] flex justify-center px-10 md:px-0'>
                            <img src={image} alt="" className='h-full py-10'/>
                        </div>
